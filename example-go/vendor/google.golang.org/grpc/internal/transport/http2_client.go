...
func (t *http2Client) Close(err error) {
	...
	fmt.Println("--- closing transport:", err)
	if t.state == closing {
		t.mu.Unlock()
		return
	}
...
func (t *http2Client) handleGoAway(f *http2.GoAwayFrame) {
	...
	fmt.Println("-- Client HandleGoAway", f)
	select {
	...
	t.prevGoAwayID = id
	fmt.Println("--- Client HandleGoAway active streams", len(t.activeStreams))
	...
	for _, stream := range streamsToClose {
		fmt.Println("----- Close stream ", stream)
